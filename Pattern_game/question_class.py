...
q = {
    "Welcome to the Game!\nThink of a design pattern and answer these following yes/no questions:\nReady? ": ["Does it provide the object creation mechanism that anhance the flexibilities of the existing code? ", "again?"],
    "Does it provide the object creation mechanism that anhance the flexibilities of the existing code? ": ["Does it ensure you have at most one instance of a class in your application? ", "Is it responsible for how one class communicates with others? "],
    ...
    "Does it explain how to assemble objects and classes into a larger structure and simplifies a structure by identifying the relationsips? ": ["Does it attach additional behavior to an object dynamically at run-time? ", "end"],
